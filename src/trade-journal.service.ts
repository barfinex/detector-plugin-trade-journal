...
export class TradeJournalService extends DetectorPluginService {
  ...
  async [PluginHook.onStart](ctx: PluginContext): Promise<void> {
    this.logger.log(
      `[onStart] ${this.name} → проверка и синхронизация позиций`,
    );
    ...
    // Определяем закрытые
    const activeTickers = new Set(activePositions.map((p) => p.instrument.symbol));

    for (const journalPos of existingPositions) {
      if (!activeTickers.has(journalPos.ticker)) {
        this.logger.log(`[onStart] Закрытие позиции ${journalPos.ticker}`);
  ...
  async findAll(filter: FilterTradeDto): Promise<TradeJournalEntity[]> {
    ...
    if (filter.studioGuid)
      qb.andWhere('trade.studioGuid = :studioGuid', {
        studioGuid: filter.studioGuid,
      });
    if (filter.detectorKey)
      qb.andWhere('trade.detectorKey = :detectorKey', {
        detectorKey: filter.detectorKey,
      });
    if (filter.ticker)
